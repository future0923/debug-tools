...
public class PropertySourceLoaderTransformer {
    ...
    private static final Logger LOGGER = Logger.getLogger(PropertySourceLoaderTransformer.class);
    ...
    public static void transformYamlPropertySourceLoader(CtClass clazz, ClassPool classPool) throws
        ...
        if (ctMethod.getParameterTypes().length == 2) {
            ...
            ctMethod.addLocalVariable(reloadVariableName(),
                classPool.get("io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v2.YamlPropertySourceLoader"));
            ctMethod.insertBefore(
                "{" + reloadVariableName() + " = new io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v2.YamlPropertySourceLoader($1, $2);}");
            ...
            ctMethod.addLocalVariable(reloadVariableName(),
                classPool.get("io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v1.YamlPropertySourceLoader"));
            ctMethod.insertBefore(
                "{" + reloadVariableName() + " = new io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v1.YamlPropertySourceLoader($1, $2, $3);}");
            ctMethod.instrument(new ExprEditor() {
                @Override
                public void edit(MethodCall m) throws CannotCompileException {
        ...
        if (ctMethod.getParameterTypes().length == 2) {
            if (isSpringBoot2LowerVersion(clazz, classPool)) {
                ...
                ctMethod.addLocalVariable(reloadVariableName(), classPool.get(
                    "io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v2.LowVersionPropertiesPropertySourceLoader"));
                ctMethod.insertBefore(
                    "{" + reloadVariableName() + " = new io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v2"
                ...
                LOGGER.debug(
                    "Patch org.springframework.boot.env.PropertiesPropertySourceLoader with 2 parameters and not "
                        + "lower version");
                ctMethod.addLocalVariable(reloadVariableName(),
                    classPool.get("io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v2.PropertiesPropertySourceLoader"));
                ctMethod.insertBefore(
                    "{" + reloadVariableName() + " = new io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v2.PropertiesPropertySourceLoader($0, "
                        + "$1, $2);}");
            }
            ctMethod.instrument(new ExprEditor() {
                @Override
            ...
            ctMethod.addLocalVariable(reloadVariableName(),
                classPool.get("io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v1.PropertiesPropertySourceLoader"));
            ctMethod.insertBefore(
                "{" + reloadVariableName() + " = new io.github.future0923.debug.tools.hotswap.core.plugin.spring.boot.env.v1.PropertiesPropertySourceLoader($1, $2, "
                    + "$3);}");
            ctMethod.instrument(new ExprEditor() {
                @Override
                public void edit(MethodCall m) throws CannotCompileException {
    ...
    private static void enhanceBasePropertySourceLoader(CtClass clazz) throws CannotCompileException {
        clazz.addMethod(CtMethod.make("private void $$ha$load0(org.springframework.core.env.PropertySource p, " +
            "io.github.future0923.debug.tools.hotswap.core.plugin.spring.api.PropertySourceReloader r) throws java.io.IOException { " +
            "if (p instanceof io.github.future0923.debug.tools.hotswap.core.plugin.spring.transformers.api.ReloadablePropertySource) { " +
            "((io.github.future0923.debug.tools.hotswap.core.plugin.spring.transformers.api.ReloadablePropertySource) p).setReload(r); " +
            "} }", clazz));
