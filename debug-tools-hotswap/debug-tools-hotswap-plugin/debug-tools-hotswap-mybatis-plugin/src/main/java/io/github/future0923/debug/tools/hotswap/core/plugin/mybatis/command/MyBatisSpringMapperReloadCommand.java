...
public class MyBatisSpringMapperReloadCommand {
    ...
    /**
     * 当class重新定义时，通过{@link io.github.future0923.debug.tools.hotswap.core.plugin.mybatis.patch.MyBatisSpringPatcher#redefineMyBatisSpringMapper}创建命令后调用这
     */
    public static void reloadConfiguration(String className) {
        try {
            MyBatisSpringMapperReload reload = new MyBatisSpringMapperReload();
            reload.reload(className);
        } catch (Exception e) {
