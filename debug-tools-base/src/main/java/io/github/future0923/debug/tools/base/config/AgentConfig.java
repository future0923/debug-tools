...
public class AgentConfig {
    ...
    private void cleanOldVersionDependencies() {
        try {
            ...
            for (File file : files) {
                if (file.isFile() && shouldDeleteFile(file, currentVersion)) {
                    try {
                        ...
                        if (deleted) {
                            logger.info("Deleted old version dependency: " + file.getName());
                        } else {
                            logger.info("Failed to delete file (may be in use): " + file.getName());
                        }
                    } catch (Exception e) {
                        logger.info("Failed to delete file (may be in use): " + file.getName());
                    }
            ...
            logger.warning("Error while cleaning old version dependencies", e);
        }
    }
