 ...
 */
public class DebugPowerClassLoaderUtils {
	...
	static {
		...
		// 加入原始类型
		primitiveTypes.addAll(DebugPowerClassUtils.BasicType.PRIMITIVE_WRAPPER_MAP.keySet());
		// 加入原始类型数组类型
	...
	public static ClassLoader getClassLoader() {
		...
		if (classLoader == null) {
			classLoader = DebugPowerClassLoaderUtils.class.getClassLoader();
			if (null == classLoader) {
	...
	private static Class<?> doLoadClass(String name, ClassLoader classLoader, boolean isInitialized){
		...
		if (name.endsWith(ARRAY_SUFFIX)) {
			...
			try {
				...
				if (null == clazz) {
					throw new RuntimeException(new ClassNotFoundException("name", ex));
				}
			}
		}
		return clazz;
